...
plot_pedigree_sims <- function(sims_dataframe, pwd_path, pair) {
  ...
  fin_rel = list(type="line", line=list(color="black"), xref='paper', yref='y', x0=0, x1=1, size=2)
  sd_plus = list(type="line", line=list(color="black", dash="dash"), xref='paper', yref='y', x0=0, x1=1, size=0.3)
  sd_min  = list(type="line", line=list(color="black", dash="dash"), xref='paper', yref='y', x0=0, x1=1, size=0.3)
  uncert  = list(type = "rect", fillcolor ="#AAAAAA", line = "#AAAAAA", opacity = 0.3, x0 = 0, x1 = 1, xref = "paper",y0 = plotdist_mean - plotdist_std, y1 = plotdist_mean + plotdist_std, yref = "y")       
  ...
  # ---- A. Main plot.
  fig <- plotly::plot_ly(type = 'violin',
                  data     = sims_dataframe,
                  x        = ~label,
                  y        = ~avg,
                  color    = ~label,
  ...
  fig <- fig %>% plotly::layout(title  = list(text = paste("Simulation results of", pair),
                                y      = 0.99,
                                yref   = "paper" 
                               ),
                                shapes = lines
  ...
  fig <- fig %>% plotly::config(editable    = FALSE,
                                displaylogo = FALSE,
                                scrollZoom  = TRUE
                               )
}