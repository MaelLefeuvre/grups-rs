...
pub fn mock_offspring(label: &str, parents_labels: Option<[&str; 2]>) -> Individual {
    ...
    let father = Arc::new(Mutex::new(mock_founder(parents_labels[0])));
    let mother = Arc::new(Mutex::new(mock_founder(parents_labels[1])));
...
pub fn mock_pedcomparison() -> PedComparison {
    let ind1 = Arc::new(Mutex::new(mock_founder("ind1")));
    let ind2 = Arc::new(Mutex::new(mock_founder("ind2")));
